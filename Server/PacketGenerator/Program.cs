	...
	class Program
	{
		...
		static void Main(string[] args)
		{
			...
			foreach (string line in File.ReadAllLines(file))
			{
				...
				if (names.Length == 0) // PERSON 무시
					continue;

				// C_(클라전송) 또는 S_(서버전송)로 시작하는것만 읽는다
				// MsgId는 CChat, 패킷 이름은 C_Chat 처럼 만들고 싶다는 코드임
				string name = names[0];
				if (name.StartsWith("S_"))
				{
					string[] words = name.Split("_"); // 언더바 찾으면 분리
